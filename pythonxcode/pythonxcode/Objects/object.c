...
try_3way_to_rich_compare(PyObject *v, PyObject *w, int op)
{
    ...
    c = try_3way_compare(v, w);
    //printf("return try_3way_compare is %d\n", c);
    if (c >= 2) {
...
do_richcmp(PyObject *v, PyObject *w, int op)
{
    ...
    if (res != Py_NotImplemented)
        return res;
    Py_DECREF(res);
    //printf("will call try_3way_to_rich_compare.\n");
...
{
    //printf(">>>>>> _PyObject_GenericGetAttrWithDict %s\n", PyString_AS_STRING(name));
    PyTypeObject *tp = Py_TYPE(obj);
    PyObject *descr = NULL;
    PyObject *res = NULL;
    descrgetfunc f;
    ...
    if (descr != NULL &&
        ...
        //printf(">>>>>> descr is not NULL\n");
        f = descr->ob_type->tp_descr_get;
        //printf(">>>>> descr name %s\n", descr->ob_type->tp_name);
...
{
    printf("call merge_list_attr attrname is %s\n", attrname);
    PyObject *list;
    int result = 0;
...
_dir_object(PyObject *obj)
{
    ...
    if (dirfunc == NULL) {
        ...
        if (PyModule_Check(obj))
            ...
            printf("call _generic_dir. \n");
            result = _generic_dir(obj);
        }
    }
    else {
        printf("dirfunc is not null.\n");
        /* use __dir__ */
