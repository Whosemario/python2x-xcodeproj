...
/* 根据slotdefs会创建对应的descriptor，这些descriptor会被装进tp_dict中，
    当对应的descriptor被调用的时候，slotdefs中的wrapperfunc将会被调用，对应
    的PyTypeObject中的tp_xx函数就会被调用。
 
    for循环出来后，会将tp_dict["__new__"] = tp_new
 */
